...
export default (function projectsModel(){
    ...
    const newProject = function (projectName){
        ...
        db.collection("users").doc(userId).set(Projects).then(()=>{
            console.log('Synchronized')}).catch(error=>console.log(error));
    ...
    return {newProject,newTask,deleteProject,renameObject,deleteTask,editTask,doesAlreadyExist,doesAlreadyExistForEdit,projectAlreadyExists,returnToDo,checkBox}
})()